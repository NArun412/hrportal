	...
	public function getMonthTimesheetData($empId,$year,$month) { //,$project_ids="" 
		...
		$select = $this->select()
				  ...
				  ->from(array('et'=>$this->_name),array(
				  	...
				  	if(et.sun_duration is null,"00:00",time_format(cast(et.sun_duration as time),"%H:%i"))sun_duration,ifnull(ts.sun_status,"no_entry")sun_status,
					if(et.mon_duration is null,"00:00",time_format(cast(et.mon_duration as time),"%H:%i"))mon_duration,ifnull(ts.mon_status,"no_entry")mon_status,
					if(et.tue_duration is null,"00:00",time_format(cast(et.tue_duration as time),"%H:%i"))tue_duration,ifnull(ts.tue_status,"no_entry")tue_status,
					if(et.wed_duration is null,"00:00",time_format(cast(et.wed_duration as time),"%H:%i"))wed_duration,ifnull(ts.wed_status,"no_entry")wed_status,
					if(et.thu_duration is null,"00:00",time_format(cast(et.thu_duration as time),"%H:%i"))thu_duration,ifnull(ts.thu_status,"no_entry")thu_status,
					if(et.fri_duration is null,"00:00",time_format(cast(et.fri_duration as time),"%H:%i"))fri_duration,ifnull(ts.fri_status,"no_entry")fri_status,
					if(et.sat_duration is null,"00:00",time_format(cast(et.sat_duration as time),"%H:%i"))sat_duration,ifnull(ts.sat_status,"no_entry")sat_status,
				  	if(et.week_duration is null,"00:00",time_format(cast(et.week_duration as time),"%H:%i"))week_duration,
